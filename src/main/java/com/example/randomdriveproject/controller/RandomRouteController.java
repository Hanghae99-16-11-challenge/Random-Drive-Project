...
import com.example.randomdriveproject.navigation.random.service.*;
...
public class RandomRouteController {
    ...
    // 랜덤 알고리즘 테스트
    @GetMapping("/dijkstra")
    public ResponseEntity<RouteResult> getRoute(@RequestParam String originAddress, @RequestParam Integer distance) {
        RouteResult response = randomAlgorithmsService.requestAllRandomWay(originAddress, distance);
        return new ResponseEntity<>(response, HttpStatus.OK);
    }
